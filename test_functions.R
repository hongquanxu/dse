...
## test functions used by Figure_S1.R
# For function details and reference information, see: Emulation/Prediction Test Problems
...
Borehole8 <- borehole1<- function(xx, extend_Kw=FALSE, log_y=TRUE, ord=1:8)
{ ## Borehole1 function; see borehole3 with rearranged variables 
## borehole2 with the range of Kw is extended as in Morris et al. (1993)
  # y  = water flow rate
  # xx = c(rw, r, Tu, Hu, Tl, Hl, L, Kw)
 
 	xx = xx[order(ord)]		# rearrange variables, 11/3/25

 	D.range=matrix(c(0.05, 0.15, 100, 50000,  63070, 115600,  990, 1110, 63.1, 116,  700, 820,   1120, 1680,  9855, 12045),2,8)
 	if(extend_Kw)  	# extend the range of Kw as in Morris et al. (1993) creating a more nonlinear and non-additive function
 	D.range=matrix(c(0.05, 0.15, 100, 50000,  63070, 115600,  990, 1110, 63.1, 116,  700, 820,   1120, 1680,  1500, 15000),2,8) 

	# scale x from [0,1] to design range
  for(j in 1:8) xx[j] = (D.range[2,j] - D.range[1,j])*xx[j] + D.range[1,j]
  ...
  y <- frac1 / frac2
  if(log_y)   y=log(y) # using log(y) is recommended by Fang et al. (2006)  
  return(y)
}


Circuit6 <- function(xx)
{  # OTL CIRCUIT FUNCTION
  # Output: Vm = midpoint voltage
  # Input: xx = c(Rb1, Rb2, Rf, Rc1, Rc2, beta)
  
  	D.range=matrix(c(50, 150, 25, 70, 0.5, 3, 1.2, 2.5, 0.25, 1.2, 50, 300), 2, 6)

# scale xx from [0,1] to design range
  for(j in 1:6) xx[j] = (D.range[2,j] - D.range[1,j])*xx[j] + D.range[1,j]
...
AllFun.lib=list(
list(name="OTL Circuit", dim=6, fun= Circuit6),
list(name="Borehole", dim=8, fun=Borehole8),
list(name="Oakley & OHagan", dim=15, fun= OakleyOhagan15)
)


show.functions=function(fun.lib=AllFun.lib)
{ # use fun.lib=Morris.lib for Morris functions
	n=length(fun.lib)
	for(i in 1:n) cat("fun id=", i, "dim=", fun.lib[[i]]$dim, "function name=", fun.lib[[i]]$name, "\n")
}
